...
import { Loader2, User, Lock, Eye, EyeOff, ArrowRight, BookOpen, ClipboardList, TrendingUp } from "lucide-react";
import Link from "next/link";

/**
 * شعار المكتبة يُعرض هنا دائمًا — إذا رُفع شعار من إعدادات الموقع
 * (logoUrl قادم من قاعدة البيانات عبر الصفحة الأب) نعرضه، وإلا نعرض
 * الأيقونة الافتراضية فقط. لا يوجد أي شعار ثابت مكتوب هنا مطلقًا.
 */
function BrandMark({
  logoUrl,
  className = "h-9 w-9",
  ...
  if (logoUrl) {
    ...
    return <img src={logoUrl} alt="" className={`${className} shrink-0 rounded-xl object-cover`} />;
  }
  return (
    <span
    ...
    >
      <BookOpen className={dark ? "h-5 w-5" : "h-4.5 w-4.5"} strokeWidth={2} />
    </span>
  );
}
    ...
    <main className="flex min-h-screen bg-paper lg:items-stretch">
      ...
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-ink px-12 py-10 text-white lg:flex xl:px-16">
        ...
        <Link href="/" className="relative flex items-center gap-2.5 font-display text-2xl font-bold">
          <BrandMark logoUrl={logoUrl} dark />
          {libraryName}
        </Link>
        ...
        <div className="relative">
          ...
          <div className="mt-8 max-w-sm space-y-4">
            {[
              { icon: BookOpen, text: "إضافة الملازم وربطها بالأستاذ المسؤول" },
              { icon: ClipboardList, text: "متابعة الطلبات لحظة بلحظة" },
              { icon: TrendingUp, text: "تقارير أرباح واضحة لكل ملزمة" },
        ...
        <p className="relative text-xs text-white/40">
          © {new Date().getFullYear()} {libraryName} — كل الحقوق محفوظة
        </p>
      </div>
